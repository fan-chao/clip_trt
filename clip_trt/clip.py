...
class CLIPModel():
    ...
    def __init__(self, model, text=True, vision=True, **kwargs):
        ...
        if 'chinese' in model.lower():
            #: The ChineseCLIPTextModel (loaded when ``text=True``)
            self.text = ChineseCLIPTextModel.from_pretrained(model, **kwargs) if text else None
        else:
        ...
        logging.debug(
            f"{self.model_name}  logit_scale={self.logit_scale}  logit_scale_exp={self.logit_scale_exp}  logit_bias={self.logit_bias}")
    ...
    def embed_image(self, image, **kwargs):
        ...
        if self.vision is None:
            raise RuntimeError(
                f"{self.model_name} should have been loaded with ``vision=True`` to use CLIPModel.embed_image()")
    ...
    def embed_text(self, text, **kwargs):
        ...
        if self.text is None:
            raise RuntimeError(
                f"{self.model_name} should have been loaded with ``text=True`` to use CLIPModel.embed_text()")
    ...
    def embed(self, inputs, normalize=True, **kwargs):
        ...
        def _embed(input, **kwargs):
            if input is None:
                ...
                return input
            elif is_image(input) or (
                    isinstance(input, str) and (input.endswith(ImageExtensions) or input.startswith('http'))):
                return self.embed_image(input, **kwargs)
            elif isinstance(input, str):
                return self.embed_text(input, **kwargs)
            else:
                raise TypeError(
                    f"CLIPModel.embed() expects inputs of type str, torch.Tensor, or np.ndarray (was {type(input)})")
